...
const GameOverOverlay = ({ data }) => {
  ...
  const isAborted = data.reason.includes("GAME ABORTED");
  
  // Theme configuration based on the outcome
  let theme = { 
      bg: "bg-gray-900", 
      text: "text-gray-200", 
      title: "GAME OVER", 
  ...
  if (isCivilianWin) {
      theme = { bg: "bg-blue-900", text: "text-blue-200", title: "VICTORY", titleColor: "text-blue-500", border: "border-blue-600" };
  } else if (isImposterWin) {
      theme = { bg: "bg-red-900", text: "text-red-200", title: "DEFEAT", titleColor: "text-red-600", border: "border-red-600" };
  }

  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/90 font-pixel">
      <div className={`w-full max-w-2xl p-1 border-4 ${theme.border} animate-in fade-in zoom-in duration-500`}>
        <div className={`${theme.bg} p-10 flex flex-col items-center text-center shadow-2xl relative overflow-hidden`}>
          ...
          <p className="text-white text-xl mb-12 font-bold bg-black/40 px-6 py-2 border-l-4 border-r-4 border-white/20">
            {data.reason.replace("CIVILIANS WIN! ", "").replace("IMPOSTER WINS! ", "").replace("GAME ABORTED: ", "")}
          </p>

          {/* IMPOSTER REVEAL CARD - HIDDEN IF GAME WAS ABORTED */}
          {!isAborted && (
          ...
          <button 
            ...
            className="group relative px-8 py-3 bg-gray-200 hover:bg-white text-black font-bold text-xl border-b-4 border-gray-500 active:border-b-0 active:translate-y-1 transition-all flex items-center gap-2 mt-4"
          >
